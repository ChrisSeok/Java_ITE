...
function myFunc() {
	...
	$.ajax({
		...
		success: function(result) {
            ...
            // 각 영화 정보를 테이블에 추가
            $.each(boxOfficeList, function(index, movie) {
				...
				var data = {
                    "query": movie.movieNm // 영화 제목으로 이미지 검색
                };
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        var imageUrl = imageInfo.thumbnail_url;
                        // 이미지를 화면에 표시
                        var image = $("<td></td>");
                        image.append($("<img>").attr("src", imageUrl))
                        .attr("alt", "영화 포스터")
                        .css("width", "30") // 가로 너비 지정
                        .css("height", "auto"); // 세로 높이는 자동으로 조절되도록 설정;
